...
class FilePlatformStore:
    ...
    def load(self) -> PlatformState:
        ...
        tools = {
            item["id"]: OpsToolDefinition.model_validate(item)
        ...
        return PlatformState(agents=agents, tools=tools, workflows=workflows, runs=runs)

    def save(self, state: PlatformState) -> None:
        payload = {
            "agents": [a.model_dump(mode="json") for a in state.agents.values()],
            "tools": [t.model_dump(mode="json") for t in state.tools.values()],
